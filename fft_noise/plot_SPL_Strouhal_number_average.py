...
def read_input_file(input_file):
    ...
    return {
        ...
        "window_type": params.get("window_type", "hann").strip().lower(),
    }
...
def build_window(window_type, n):
    if window_type == "hann":
        window = np.hanning(n)
    elif window_type == "none":
        window = np.ones(n, dtype=float)
    else:
        raise ValueError(f"Unsupported window_type: {window_type}. Use 'hann' or 'none'.")
    return window

def compute_spl_spectrum(p, dt, p_ref, window_type="none"):
    fs = 1.0 / dt
    n = len(p)

    p_fluc = p - np.mean(p)

    window = build_window(window_type, n)
