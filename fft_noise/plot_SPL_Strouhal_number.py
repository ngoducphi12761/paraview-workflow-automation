...
def analyze_from_config(config):
    ...
    i_peak = np.argmax(spl_sel)
    f_peak = freq_sel[i_peak]
    st_peak = st_sel[i_peak]
    spl_peak = spl_sel[i_peak]

    output_png_path = file_path.parent / output_png
    output_dat_path = file_path.parent / output_dat

    export_data = np.column_stack((freq_sel, st_sel, spl_sel, amp_sel))
    np.savetxt(
        output_dat_path,
        export_data,
        fmt="%.8e",
        header="frequency_Hz Strouhal SPL_dB p_rms_Pa",
        comments=""
    )

    plt.figure(figsize=(9, 5))
    plt.plot(st_sel, spl_sel, linewidth=1.0)
    plt.scatter([st_peak], [spl_peak], color="red", label=f"Peak St = {st_peak:.5f}")
    ...
    print(f"Dominant peak: f = {f_peak:.3f} Hz, St = {st_peak:.5f}, SPL = {spl_peak:.3f} dB")
    print(f"Saved plot to: {output_png_path}")
    print(f"Saved data to: {output_dat_path}")
